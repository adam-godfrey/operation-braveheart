...
class News_Model extends Model {
	...
	public function newsContent($id) {
		...
		$sql = 'SELECT id, title, content, author, image, alternate, keywords FROM news WHERE id = :id LIMIT 1';
	...
	public function newsDelete($id) {
		...
		return array('result' => $this->db->delete('news', "`id` IN (".implode(',',$id).")"), 'action' => 'deleted');
	}
	
	public function newsArchive($id) {
	
        return array('result' => $this->db->updateCase('news', 'archived', "`id` IN (".implode(',',$id).")"), 'action' => 'archived');
	}
}