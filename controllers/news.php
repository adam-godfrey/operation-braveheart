...
class News extends Controller {
    ...
    public function index() {
		...
		if(!empty($rows)) {
			...
			$this->view->news = $rows;
			// create the paging 
			$this->view->paging = ($rowcount > $this->entries_per_page) ? $this->model->paging($total_pages, $page, 'news', 'pages') : '';
	...
	public function view($arg = false) {
		...
		$nav_array = array(array("url" => URL, "name" => "Home"),
						   array("url" => URL . "news", "name" => "News"),
						   array("url" => URL . "news/" . str_replace(' ', '-', strtolower($rows[0]['title'])), "name" => ucwords(strtolower($rows[0]['title'])))
		);
		...
		$this->view->news_title = $rows[0]['title'];
	...
	public function reply($arg = false) {
		...
		// get the rows returned from database
		extract($this->model->getNews($this->entries_per_page, $arg));	
		...
		// set the comments count
		$this->view->comm_count = $this->model->getCommentsCount($rows[0]['id']);
		// if there are comments in the database for this news
		if($this->model->getCommentsCount($rows[0]['id']) > 0) {
			// extract it from the database
			extract($this->model->getComments($rows[0]['id']));
			// and set it so they can be viewed
